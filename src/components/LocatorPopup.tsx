				...
				<Marker position={station.latLong} key={`station-${station.id}`}>
					<Popup>
						<svg
							...
							width="16"
							height="16"
							fill="currentColor" // TEMP
							className={`bi bi-star` + props.isFavorite(station.id) ? `-fill` : ''}
							viewBox="0 0 16 16"
							onClick={() => props.handleFavoriteChange(station, props.isFavorite(station.id))}
						>
							{props.isFavorite(station.id) ? (
